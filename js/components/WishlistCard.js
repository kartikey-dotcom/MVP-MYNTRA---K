 ...
 * Displays wishlisted item with 'STYLESTUDIO HERO' badge on the active hero piece,
 * remove button '✕', and 'MOVE TO BAG' button.
 */
  ...
  return `
    <article class="desktop-wishlist-card ${isCurrentHero ? 'is-active-hero' : ''}" data-item-id="${item.id}">
      ...
      <div class="card-info" data-action="select-hero" data-item-id="${item.id}">
        ...
        <p class="card-product-title" title="${item.title || item.name}">${item.title || item.name}</p>
